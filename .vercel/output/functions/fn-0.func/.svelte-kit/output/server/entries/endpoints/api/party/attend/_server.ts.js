...
const POST = async ({ request, locals, url }) => {
  ...
  if (status === "request" && ["yes", "maybe"].includes(currentStatus))
    return new Response("OK");
  const alert = status === "yes" ? "is attending" : status === "maybe" ? "might attend" : status === "no" ? "is not attending" : "";
  ...
  const update = {
    [`attendees.${uid}`]: {
      ...
      alerted: status === "yes" || void 0
    },
    guests: ["yes", "maybe"].includes(status) ? FieldValue.arrayUnion(uid) : FieldValue.arrayRemove(uid)
  };
  if (isHost || locals.session?.uid === uid) {
    await ref.update(update);
    if (alert)
      await addMessage({ uid, alert, partyId });
